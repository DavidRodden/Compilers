...
public class Sem1Visitor extends ASTvisitor {
	...
	protected void initGlobalSymTab() {
		...
		addDummyMethod(classStringDecl, "concat", "String", new String[] { "String" });
		addDummyMethod(classStringDecl, "substring", "String", new String[] { "int", "int" });
		addDummyMethod(classStringDecl, "length", "int", new String[] {});
		addDummyMethod(classStringDecl, "charAt", "int", new String[] { "int" });
		addDummyMethod(classStringDecl, "compareTo", "int", new String[] { "String" });
	...
	protected static void addDummyMethod(ClassDecl dec, String methName, String rtnTypeName, String[] parmTypeNames) {
	...
	public Object visitClassDecl(ClassDecl n) {
		...
		currentClass = n;
		return super.visitClassDecl(currentClass);
	}

	public Object visitInstVarDecl(InstVarDecl n) {
		if (!currentClass.instVarTable.containsKey(n.name)) currentClass.instVarTable.put(n.name, n);
		else errorMsg.error(n.pos, "Duplicate instance variable declaration: " + n.name);
		return null;
	}
	

	public Object visitMethodDecl(MethodDecl n){
		if(!currentClass.methodTable.containsKey(n.name))	currentClass.methodTable.put(n.name, n);
		else errorMsg.error(n.pos, "Duplicate method declaration: " + n.name);
		return null;
	}
}
